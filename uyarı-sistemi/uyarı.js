...
exports.run = async (client, message, args) => {
    let uarılog = await db.fetch(`CwlUyarıLog_${message.guild.id}`)
    ...
    if(!(user||sebebmoruk)) return message.reply(`Yanlış Kullanım! Örn: \`c!uyarı @Kullanıcı [sebep]\``)

    let uyarısayı = await db.fetch(`CwlUyarıKullanıcı_${message.guild.id}_${user.id}`)
    if(!uyarısayı) { // Kullanıcı Hiç Uyarılmadıysa
        db.set(`CwlUyarıKullanıcı_${message.guild.id}_${user.id}`, 1) // Database'e kaydettik  **${user}** İsimli Kullanıcı ${sebebmoruk} Sebebi İle Uyarıldı!
        ...
        db.add(`CwlUyarıKullanıcı_${message.guild.id}_${user.id}`, 1)
        ...
        .setColor(`WHITE`)
        .addField("Uyarı-Log", `\n**${message.author}** İsimli Yetkili **${user}** İsimli Kullanıcıyı **${sebebmoruk}** Sebebi İle Uyardı!\nBu Kullanıcının Toplamda **${uyarısayı}** Uyarısı Var!`)
        .setFooter("© 2020 Curwels", client.user.avatarURL())
        .setTimestamp()
    ...
    if(uyarısayı >= 3) {
        ...
        db.delete(`CwlUyarıKullanıcı_${message.guild.id}_${user.id}`)
    }
}
